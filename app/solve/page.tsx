...
// 讲题语义的 requirement 框架：把大纲生成锚定为"单题精讲"而非"讲主题"，
// 并强约束忠实读题（防止把图里的题脑补成同学科的另一道"典型题"）。
const SOLVE_REQUIREMENT_TEMPLATE = `这是一道具体题目的照片，请做"单题精讲"，不是一门主题课程。

第一步，忠实读题：逐字识别照片里的题目——已知条件、所求问题、关键数字与图形都要准确读出。整堂课自始至终只围绕照片里的这一道题展开，严禁替换成"类似题"或凭印象编造的"典型题"；如果照片信息不完整，就如实说明缺失部分，不要脑补。

第二步，围绕这道题规划紧凑的讲解结构（建议 4-6 个场景）：
1) 先讲解解这道题所需的前置知识点；
2) 再一步步讲解这道题的解题过程；
3) 在适合的地方用交互式演示（如仿真/图示/3D）帮助理解；
4) 最后用一道小测检验对这道题相关知识点的掌握。

不要使用 PBL 项目式场景。课程标题用这道题本身的主题，而不是宽泛的学科名。`;
